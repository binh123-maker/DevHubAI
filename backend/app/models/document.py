...
from app.models.enums import DocumentStatus, KanbanStatus
...
class Document(Base):
    ...
    kanban_status: Mapped[KanbanStatus] = mapped_column(
        Enum(KanbanStatus, name="kanban_status", values_callable=lambda x: [e.value for e in x]),
        nullable=False,
        default=KanbanStatus.NEW,
    )
    kanban_updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False
    )
...
class DocumentChunk(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_document_chunks_version_index", "document_version_id", "chunk_index"),
    )
